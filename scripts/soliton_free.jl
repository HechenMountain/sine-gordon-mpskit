...
#          scripts/run_soliton_free.slurm
#
...
# an automatic requeue picks up where it left off, however many days later) or a
# fresh dated one.  A resume then auto-triggers exactly when a state checkpoint
...
const STATE_EVERY = env_get("STATE_EVERY", 10)  # save full MPS state every N TDVP steps
...
const RUN_DIR    = resolve_run_dir(joinpath(DATA_DIR, "soliton_free"))
const STATE_PATH = joinpath(RUN_DIR, "ckpt_state_$(tag).jld2")
const RESUMING   = isfile(STATE_PATH)

# Logging

log_path = init_log("soliton_free")
vcapture() do
...
vprint("═"^80)
vprint("  Free soliton wavepacket propagation  ")
vprint("-"^80)
...
vprint("  Wavepacket parameters : χ = $χ_val, p0 = $p0_val, σ = $σ_val, x_K = $x_K")
...
if RESUMING
    ...
    flush(stdout)
    # ϕ_L/ϕ_R are restored from the checkpoint's stored scalars inside
    # run_scattering, so they need not be passed here.
    ...
    # Step 2 — Build soliton wavepacket
    vprint("\n[2/3] Building soliton wavepacket ...")
    flush(stdout)
    result = @timed soliton_wavepacket(;
        H       = vac0.H,
        ψ_vac0  = vac0.psi,
        ψ_vac1  = vac1.psi,
        N=N_val, d=d_val, a=a_val, m=m_val, β=β_val,
        x_K, p0=p0_val, σ=σ_val, χ=χ_val,
        verbosity=1,
    )
    wp       = result.value
    elapsed  = result.time
    ...
    # carried into the resume checkpoint via `extras` (alongside M_K), so a
    # resumed run needs no VUMPS or excitation solve.  A pure vacuum-0 reference
    # (ϕ_L = ϕ_R = 0) is the correct baseline for BOTH chain ends: the endpoint
    # energy density is ϕ-independent and vacuum 0/1 are degenerate, so it
    # cancels the vac-1 right edge too.
    ε_vac = vacuum_energy_density(vac0.psi, N_val; d=d_val, a=a_val, m=m_val, β=β_val)

    # Step 3 — Run propagation (TDVP time evolution)
    ...
    result = @timed run_scattering(ψ, H_finite;
        ...
        checkpoint_dir = RUN_DIR,
        checkpoint_tag = tag,
        # Ghost boundary values used in H_finite (Q = +1 sector: ϕ_L ≈ 0,
        # ϕ_R ≈ 2π/β) — keeps the energy-density split consistent with ⟨H⟩.
        ϕ_L     = wp.ϕ_L,
        ϕ_R     = wp.ϕ_R,
        state_every = STATE_EVERY,
        extras      = (; ε_vac, M_K),
...
# checkpoint), so it is identical on fresh and resumed runs.  A pure vacuum-0
# reference (ϕ_L = ϕ_R = 0) is the correct baseline for BOTH chain ends: the
# endpoint energy density is ϕ-independent and vacuum 0/1 are degenerate, so it
# cancels the vac-1 right edge too.
...
vprint("  DONE")
vprint("═══════════════════════════════════════════════════════════")
vprint(@sprintf("  Soliton mass M_soliton = %.6f\n", result.extras.M_K))
